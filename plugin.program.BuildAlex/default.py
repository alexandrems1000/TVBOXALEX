...
EXCLUDES     = ['plugin.program.BuildAlex','script.module.addon.common']
...
def COMMUNITY_BACKUP():
    ...
    exclude_dirs_full =  ['plugin.program.BuildAlex']
    exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
    exclude_dirs =  ['plugin.program.BuildAlex', 'cache', 'system', 'Thumbnails', "peripheral_data",'library','keymaps']
...
def ARCHIVE_FILE(sourcefile, destfile):
    ...
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.BuildAlex' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def WipeXBMC():
    if skin!= "skin.confluence":
        ...
        if choice == 1:
            ...
            title = urllib.quote_plus(vq)
            backup_zip = xbmc.translatePath(os.path.join(mybackuppath,title+'.zip'))
            exclude_dirs_full =  ["plugin.program.BuildAlex", "plugin.video.adrxbmccustombuildswizard"]
            exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
            message_header = "Creating full backup of existing build"
            message1 = "Archiving..."
            message2 = ""
...
def FRESHSTART(params):
    if skin!= "skin.confluence":
        dialog.ok('[COLOR=blue][B]AMO[/B][/COLOR][COLOR=green]box[/COLOR][COLOR=white] Wizard ','Please switch to the default Confluence skin','before performing a wipe.','')
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
        return
    else:
    ...
    if choice2 == 0:
        ...
        dp.create("[COLOR=blue][B]AMO[/B][/COLOR][COLOR=green]box[/COLOR][COLOR=white] Wizard","Wiping Install",'', 'Please Wait')
        try:
            for root, dirs, files in os.walk(HOME,topdown=True):
                dirs[:] = [d for d in dirs if d not in EXCLUDES]
                for name in files:
    ...
    REMOVE_EMPTY_FOLDERS()
    REMOVE_EMPTY_FOLDERS()
    dialog.ok('[COLOR=blue][B]AMO[/B][/COLOR][COLOR=green]box[/COLOR][COLOR=white] Wizard','Wipe Successful, please restart XBMC/Kodi for changes to take effect.','','')
    killxbmc()
...
def IPCHECK(url='http://www.iplocation.net/',inc=1):
    ...
    for ip, region, country, isp in match:
        if inc <2: dialog=xbmcgui.Dialog(); dialog.ok('[COLOR blue][B]AMO[/B][/COLOR][COLOR green]box[/COLOR][COLOR gold] Wizard',"[B][COLOR gold]Your IP Address is: [/COLOR][/B] %s" % ip, '[B][COLOR gold]Your IP is based in: [/COLOR][/B] %s' % country, '[B][COLOR gold]Your Service Provider is:[/COLOR][/B] %s' % isp)
        inc=inc+1
