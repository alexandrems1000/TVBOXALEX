...
addon_id = 'plugin.program.maniac'
ADDON = xbmcaddon.Addon(id=addon_id)
AddonID='plugin.program.maniac'
AddonTitle="[COLOR red][B]Team[/B] ManiaC[/COLOR] [COLOR gold]Wizard[/COLOR]"
...
EXCLUDES     = ['plugin.program.maniac','script.module.addon.common']
...
def BUILDMENU():
    link = OPEN_URL('hhttp://pastebin.com/raw/wp23n87a').replace('\n','').replace('\r','')
...
def COMMUNITY_BACKUP():
    ...
    exclude_dirs_full =  ['plugin.program.maniac']
    exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
    exclude_dirs =  ['plugin.program.maniac', 'cache', 'system', 'Thumbnails', "peripheral_data",'library','keymaps']
...
def ARCHIVE_FILE(sourcefile, destfile):
    ...
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.maniac' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def WipeXBMC():
    if skin!= "skin.confluence":
        ...
        if choice == 1:
            ...
            title = urllib.quote_plus(vq)
            backup_zip = xbmc.translatePath(os.path.join(mybackuppath,title+'.zip'))
            exclude_dirs_full =  ["plugin.program.maniac", "plugin.video.adrxbmccustombuildswizard"]
            exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf']
            message_header = "Creating full backup of existing build"
            message1 = "Archiving..."
            message2 = ""
